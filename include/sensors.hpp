...
struct quad_t {
  ...
  Encoder enc;
  /** The ports the encoder is connected to, in order of top, then bottom, when
   * the removable cover is facing up */
  unsigned char ports[2];
  /** The relative zero from which the encoder's value will be returned. Can be
...
struct pot_t {
  ...
  float request;
  /** The initialization funtion for the potentiometer, which must be called in
   * initialize() */
  void init(void);
